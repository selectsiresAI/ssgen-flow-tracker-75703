...
ALTER TABLE public.service_orders
  ADD COLUMN IF NOT EXISTS valor_total_override numeric(14,2);

DROP VIEW IF EXISTS public.v_ready_to_invoice;
DROP VIEW IF EXISTS public.v_billing_summary;
DROP VIEW IF EXISTS public.v_billing_monthly;
DROP VIEW IF EXISTS public.v_billing_by_rep;
DROP VIEW IF EXISTS public.v_billing_by_coord;

CREATE VIEW public.v_billing_summary AS
WITH base_billing AS (
  SELECT
    ...
    c.representante,
    c.coordenador,
    COALESCE(so.valor_total_override, COALESCE(so.numero_amostras, 0) * 150.00) as valor_estimado
...
SELECT
  ...
  CASE WHEN COUNT(DISTINCT id) > 0 THEN ROUND(SUM(valor_estimado) / COUNT(DISTINCT id), 2) ELSE 0 END as ticket_medio,
...
CREATE VIEW public.v_ready_to_invoice AS
SELECT
  so.id,
...
CREATE VIEW public.v_billing_monthly AS
WITH monthly_data AS (
  SELECT
    ...
    COALESCE(SUM(COALESCE(so.valor_total_override, COALESCE(so.numero_amostras, 0) * 150.00)), 0) as valor_faturado
  FROM public.service_orders so
  WHERE so.dt_faturamento IS NOT NULL
...
CREATE VIEW public.v_billing_by_rep AS
SELECT
  c.representante,
  COUNT(DISTINCT so.id) as total_ordens,
  COALESCE(SUM(so.numero_amostras), 0) as total_amostras,
  COALESCE(SUM(COALESCE(so.valor_total_override, COALESCE(so.numero_amostras, 0) * 150.00)), 0) as valor_total
FROM public.service_orders so
JOIN public.clients c ON c.ordem_servico_ssgen = so.ordem_servico_ssgen
WHERE so.dt_faturamento IS NOT NULL
...
CREATE VIEW public.v_billing_by_coord AS
SELECT
  c.coordenador,
  COUNT(DISTINCT so.id) as total_ordens,
  COALESCE(SUM(so.numero_amostras), 0) as total_amostras,
